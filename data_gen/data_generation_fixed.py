...
def resize_to_shape(matrix, shape=(600, 600)):
    y, x = matrix.shape
    target_y, target_x = shape
    resized = np.zeros(shape)
    copy_y = min(y, target_y)
    copy_x = min(x, target_x)
    resized[:copy_y, :copy_x] = matrix[:copy_y, :copy_x]
    return resized
...
def generate_maps(spice_file, voltage_file, output_dir):
    ...
    np.savetxt(os.path.join(output_dir, f"current_map_{base}.csv"), resize_to_shape(current_map), delimiter=",")
    np.savetxt(os.path.join(output_dir, f"pdn_density_map_{base}.csv"), resize_to_shape(density_map), delimiter=",")
    np.savetxt(os.path.join(output_dir, f"voltage_source_map_{base}.csv"), resize_to_shape(voltage_source_map), delimiter=",")
    np.savetxt(os.path.join(output_dir, f"ir_drop_map_{base}.csv"), resize_to_shape(ir_drop_map), delimiter=",")
